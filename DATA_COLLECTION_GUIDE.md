...
机械臂不在参考位姿时，`Start Episode` 会保持禁用。可以手动将机械臂移动到参考位姿。只有在确认当前 CAN 确实连接到需要控制的机械臂后，才使用 `Reset Home`。
...
- [ ] 已抽查回放。
- [ ] `validate_piper_data.py` 显示 `Dataset PASS`。
- [ ] 只导出本批次的新 episode。
